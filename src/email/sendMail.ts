...
const adminSubject = process.env.SUBJECT2 as string;
...
export = {
  ...
  sendAdminEmail(from: string, to: string, subject: string, html: string): Promise<unknown> {
    return new Promise((resolve, reject) => {
      transport.sendMail({ from: fromUser, subject: adminSubject, to, html }, (err, info) => {
        if (err) reject(err);
        resolve(info);
      });
    });
